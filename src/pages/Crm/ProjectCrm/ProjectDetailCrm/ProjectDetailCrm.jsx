...
import { Image, RefreshControl, ScrollView, StyleSheet, Text, TextInput, ToastAndroid, TouchableOpacity, View, Dimensions } from 'react-native'
...
const Title = ({ title }) => {
    ...
    if (title != null) {
        return <>
            <TouchableOpacity  >
                <Text style={{ color: '#000' }} numberOfLines={parseFloatOfLines} onPress={() => setparseFloatOfLines(parseFloatOfLines == 1 ? undefined : 1)} >{attributeTitlte(title)}</Text>
            </TouchableOpacity>
            <Gap height={2} />
        </>
    }
                ...
                {!loading && (
                    <>
                        <View style={{ justifyContent: 'center', alignItems: 'center' }} >
                            <Tab containerStyle={{ justifyContent: 'center', alignItems: 'center', width: '100%', flex: 1, alignSelf: 'center', alignContent: 'center', }} dense={true} scrollable={true} value={indexActive} onChange={(item) => scrollToIndex(item)} indicatorStyle={{ backgroundColor: colors.buttonColorSecond }} titleStyle={{ color: colors.textColor }}>
                        ...
                        >
                            ...
                            >
                                ...
                                {groupsTab?.length > 0 && groupsTab?.map((group, index) => {
                                    ...
                                    if (index != 0 && index != (groupsTab.length - 1)) {
                                        return (
                                            <View style={{ width: width }} >
                    ...
                    {comments?.length > 0 && comments.map((item, index) => (
                        <View>
                            <Row >
                                ...
                                <HTML key={index} source={{ html: item.comment }} enableExperimentalMarginCollapsing={true} />
                            </Row>
                        </View>
                    ))}
                </ScrollView>
